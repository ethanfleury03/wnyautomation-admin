...
const productionClerkProxyUrl = 'https://wnyautomation.com/clerk-prod-proxy';
const stagingSharedClerkProxyUrl = 'https://wnyautomation.com/clerk-proxy';

export function getClerkProxyUrl() {
  if (process.env.APP_ENV === 'staging') return stagingSharedClerkProxyUrl;
  ...
  return productionClerkProxyUrl;
}
...
function assertAllowedClerkProxyUrl(name: string, value: string) {
  ...
  const isSharedProductionClerkProxy =
    url.hostname === 'wnyautomation.com' && (url.pathname === '/clerk-proxy' || url.pathname === '/clerk-prod-proxy');
